 ...
 * @author Alexander
 */
@WebServlet(name = "LoadGraficoMarcasEntrada", urlPatterns = {"/LoadGraficoMarcasEntrada"})
public class LoadGraficoMarcasEntrada extends BaseServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (int horaDesde = 7; horaDesde <= 23; horaDesde++){
                ...
                int totalMarcas = 0;
                try{
                    totalMarcas = graficosdao.getTotalMarcas(fechaFormat.format(calendarDesde.getTime()), 
                        startTime+":00", 
                        endTime+":00",1);
                }catch(Exception ex1){
                ...
                empObj.put("num_marcas", totalMarcas);
            ...
            System.err.println("[GestionWeb.LoadGraficoMarcasEntrada]error 2: "+ex.getMessage());
            ex.printStackTrace();
        }catch (Exception ex) {
            System.err.println("[GestionWeb.LoadGraficoMarcasEntrada]error 3: "+ex.getMessage());
            ex.printStackTrace();
        }
    }
