...
public class ProveedoresCorreo extends BaseServlet {
    protected static Logger m_logger = Logger.getLogger("gestionweb");

    private static final String CRUD_TABLE_NAME     = "ProveedoresCorreo";
    private static final String CRUD_SERVLET_NAME   = "[ProveedoresCorreo.servlet]";
    private static final String FORWARD_PAGE        = "cruds/proveedores_correo.jsp";
    ...
    */
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ProveedorCorreoDAO claseDao = new ProveedorCorreoDAO();
        ...
        if(request.getParameter("action") != null){
            ...
            ProveedorCorreoVO registro = new ProveedorCorreoVO();
            if(request.getParameter("id") != null){
                registro.setCode(request.getParameter("id"));
            }
            if(request.getParameter("nombre") != null){
                registro.setName(request.getParameter("nombre"));
            }
            if(request.getParameter("dominio")!=null){
                registro.setDomain(request.getParameter("dominio"));
            }
                       
            if (action.compareTo("list") == 0) {
                System.out.println(WEB_NAME + CRUD_SERVLET_NAME + "CRUD - " + CRUD_TABLE_NAME
                    + ". Mostrar registros");
                forwardToCRUDPage(request, response, claseDao, filtroVO);   
            }else if (action.compareTo("create") == 0) {
                    System.out.println(WEB_NAME + CRUD_SERVLET_NAME + "CRUD - " + CRUD_TABLE_NAME
                        + ". Insertar registro");
                    
                    ResultCRUDVO doCreate = claseDao.insert(registro);					
                    forwardToCRUDPage(request, response, claseDao, filtroVO);
            }else if (action.compareTo("update") == 0) {  
                    System.out.println(WEB_NAME + CRUD_SERVLET_NAME + "CRUD - " + CRUD_TABLE_NAME
                        + ". Modificar registro");
                    ResultCRUDVO doUpdate = claseDao.update(registro);
                    forwardToCRUDPage(request, response, claseDao, filtroVO);
            }else if (action.compareTo("delete") == 0) {  
                    System.out.println(WEB_NAME + CRUD_SERVLET_NAME + "CRUD - " + CRUD_TABLE_NAME
                        + ". Eliminar registro");
                    ResultCRUDVO doDelete = claseDao.delete(registro.getCode());
                    forwardToCRUDPage(request, response, claseDao, filtroVO);
            }
    ...
    */
    private void forwardToCRUDPage(HttpServletRequest _request, 
        ...
        ProveedorCorreoDAO _dao,
        FiltroBusquedaCRUDVO _filtroBusqueda){
    
        try {
            List<ProveedorCorreoVO> listaObjetos = new ArrayList<>();
            System.out.println(WEB_NAME + CRUD_SERVLET_NAME + "CRUD - " + CRUD_TABLE_NAME
                + ". filtroNombre: " + _filtroBusqueda.getNombre());
            listaObjetos = _dao.getProveedores(_filtroBusqueda.getNombre(), 0, 0, "provider_name");
