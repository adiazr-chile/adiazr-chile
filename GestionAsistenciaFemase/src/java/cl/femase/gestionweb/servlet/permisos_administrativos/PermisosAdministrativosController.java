...
@WebServlet(name = "PermisosAdministrativosController", urlPatterns = {"/servlet/PermisosAdministrativosController"})
public class PermisosAdministrativosController extends BaseServlet {
    protected static Logger m_logger = Logger.getLogger("gestionweb");

    private static final long serialVersionUID = 995L;
    
    public PermisosAdministrativosController() {
...
//            
            ...
            if (action.compareTo("list") == 0) {
                try{
                    ...
                    if (intCencoId != -1){
                        if (tipo == null){
                            listaObjetos = permisosDao.getResumenPermisosAdministrativos(paramEmpresa, 
                            ...
                            objectsCount = permisosDao.getResumenPermisosAdministrativosCount(paramEmpresa, 
                                request.getParameter("rutEmpleado"),
                                paramAnio,
                                Integer.parseInt(cencoId));
                        }
                        
                        session.setAttribute("resumenPA|"+userConnected.getUsername(), listaObjetos);
                    }
                    //Convert Java Object to Json
                    JsonElement element = gson.toJsonTree(listaObjetos,
                        new TypeToken<List<PermisoAdministrativoVO>>() {}.getType());
