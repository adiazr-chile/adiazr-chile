...
<%
    ...
    System.out.println("[FemaseWeb.PESP.solicitud.jsp]"
...
<head>
    ...
    <title>Solicitudes de Permisos examen salud preventiva propias</title>
    ...
    <script type="text/javascript">
        ...
        function setCenco(cencoId){
            $('#SolicitudesTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').hide();
            if (cencoId === '-1'){
                $('#SolicitudesTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').hide();
            }else {
                $('#SolicitudesTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').show();
            }
        }
...
<body>
    <div class="site-container">
        <div class="main-header" style="position: relative">
            <h1>Solicitudes de Permisos Examen Salud Preventiva (propias)</h1>
            <h2>Realice la b&uacute;squeda, seleccionando centro de costo y rango de fechas. Luego presione "Buscar".</h2>
        </div>
...
<!-- paramRutEmpleado-->
    <form>
        ...
        <label>Permisos examen salud preventiva, desde:
            <input name="startPESP" type="text" id="startPESP">
            hasta <input name="endPESP" type="text" id="endPESP">
        </label> 
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#SolicitudesTableContainer').jtable({       
            title: 'Solicitudes existentes',
            paging: true, //Enable paging
            pageSize: 10, //Set page size (default: 10)
            sorting: true, //Enable sorting
            defaultSorting: 'solicitud.solic_fec_ingreso desc', //Set default sorting
            actions: {
                listAction: '<%=request.getContextPath()%>/servlet/SolicitudPESPController?action=listPropias&source=solicitante',
                //createAction:'<%=request.getContextPath()%>/servlet/SolicitudVacacionesController?action=create',
                <% if (!readOnly){%>
                    updateAction:'<%=request.getContextPath()%>/servlet/SolicitudPESPController?action=updatePropias'
                <%}%>
            },
            fields: {
		id:{
		...
		runEmpleado: {
                    ...
                    input: function (data) {
                        if (data.record) {
                            return '<input type="text" id="rutEmpleado" name="rutEmpleado" style="width:100px" value="' + data.record.runEmpleado + '" readonly />';
                        } 
                    }
                },
                nombreEmpleado: {
		...
		fechaHoraApruebaRechaza: {
                ...
                Accion:{
                    ...
                    options: function(data){
                        ...
                        var userSolicita = data.record.usernameSolicita;
                        return '<%=request.getContextPath()%>/LoadItems?type=acciones_solicitud&estadoSolicitud='+auxEstadoId+'&userSolicita='+userSolicita;
                    },
                    create:false,
                    list: false,
                    edit:true,
        ...
        $('#LoadRecordsButton').click(function (e) {
            e.preventDefault();
            $('#SolicitudesTableContainer').jtable('load', {
                cencoId: $('#cencoId').val(),
                startPESP: $('#startPESP').val(),
                endPESP: $('#endPESP').val()
            });
        });
