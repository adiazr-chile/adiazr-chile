...
<head>
    ...
    <title>Resumen - Permisos Examen Salud Preventiva</title>
    ...
    <script type="text/javascript">
        ...
        $('#PESPTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').hide();
        /*
        function showHideButtonNewRow(){
            var rutSelected = $("select#rut").val();
            if (rutSelected !== '-1'){
                $('#PESPTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').show();
            }else{
                $('#PESPTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').hide();
            }
        }
...
<body>
    <div class="site-container">
        <div class="main-header" style="position: relative">
            <h1>Resumen de Permisos Examen Salud Preventiva</h1>
...
<div id="PESPTableContainer"></div>
<script type="text/javascript">

    $(document).ready(function () {
        //showHideButtonNewRow();
        $('#PESPTableContainer').jtable({       
            title: 'Permisos Examen Salud Preventiva',
            paging: true, //Enable paging
            pageSize: 10, //Set page size (default: 10)
            sorting: true, //Enable sorting
            defaultSorting: 'run_empleado ASC', //Set default sorting
            actions: {
                listAction: '<%=request.getContextPath()%>/servlet/PermisoExamenSaludPreventivaController?action=list'
            },
            fields: {
               rowKey: {
                    title:'Row key',
            ...
            toolbar: {
                    ...
                    items: [{
                        ...
                        text: 'CSV',
                            click: function () {
                                document.location.href='<%=request.getContextPath()%>/DataExportServlet?type=resumenPESPCSV';
                            }
                        }
                    ]
            },
        ...
        $('#LoadRecordsButton').click(function (e) {
            e.preventDefault();
            $('#PESPTableContainer').jtable('load', {
                cencoId: $('#cencoId').val(),
                rutEmpleado: $('#rut').val(),
