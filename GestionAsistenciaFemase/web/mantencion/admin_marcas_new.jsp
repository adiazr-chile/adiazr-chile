...
<div class="container-fluid my-4">
    ...
    <!-- botones principales: nueva marca + export -->
    <div class="d-flex justify-content-between mb-2">
        <button id="nuevoBtn" class="btn btn-primary btn-sm">
            <span class="material-icons" style="font-size:1em;">add</span> Nueva marca
        </button>
...
<div class="modal fade" id="newModal" tabindex="-1" aria-labelledby="newModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <form id="newForm" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="newModalLabel">Nueva marca</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body row g-2">

        <input type="hidden" id="newNombreEmpleado" name="nombreEmpleado">
        <input type="hidden" id="newHashcode" name="hashcode">

        <div class="col-md-4">
          <label class="form-label">Rut</label>
          <input type="text" class="form-control" id="newRutEmpleado" name="rutEmpleado" readonly>
        </div>

        <div class="col-md-4">
          <label class="form-label">Fecha marcación</label>
          <input type="date" class="form-control" id="newFecha" name="fecha">
        </div>

        <div class="col-md-4">
          <label class="form-label">Hora marcación</label>
          <input type="time" class="form-control" id="newHora" name="hora" step="1">
        </div>

        <div class="col-md-4">
          <label class="form-label">Tipo</label>
          <select class="form-select" id="newTipo" name="tipo">
        ...
        <div class="col-md-4">
          <label class="form-label">Tipo marca manual</label>
          <select class="form-select" id="newTipoMarcaManual" name="tipoMarcaManual">
            <option value="">-- Seleccione --</option>
            <option value="1">Olvido marca</option>
            <option value="2">Reloj con Problemas</option>
            <option value="3">Corte Energía Eléctrica</option>
        ...
        <div class="col-12">
          <label class="form-label">Comentario</label>
          <textarea class="form-control" id="newComentario" name="comentario" rows="2"></textarea>
        </div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        // Nueva marca: abrir modal
        $('#nuevoBtn').on('click', function () {
            const rutSeleccionado = $('#empleado').val();

            if (!rutSeleccionado) {
                showToast('Debe seleccionar un empleado antes de crear una marca.', 'error');
                return;
            }

            $('#newRutEmpleado').val(rutSeleccionado);
            ...
            $('#newFecha').val(`${yyyy}-${mm}-${dd}`);
            $('#newHora').val(`${hh}:${mi}:${ss}`);

            $('#newTipo').val('');
            $('#newTipoMarcaManual').val('');
            $('#newComentario').val('');
            $('#newNombreEmpleado').val('');
            $('#newHashcode').val('');

            let modal = new bootstrap.Modal(document.getElementById('newModal'));
            modal.show();
        });
        ...
        $('#newForm').on('submit', function (e) {
            ...
            const rut        = $('#newRutEmpleado').val();
            const fecha      = $('#newFecha').val();
            const hora       = $('#newHora').val();
            const tipo       = $('#newTipo').val();
            const tipoManual = $('#newTipoMarcaManual').val();

            if (!rut)   { showToast('Rut es obligatorio.', 'error'); return; }
            if (!fecha) { showToast('Fecha es obligatoria.', 'error'); return; }
            if (!hora)  { showToast('Hora es obligatoria.', 'error'); return; }
            if (!tipo)  { showToast('Debe seleccionar Tipo (Entrada/Salida).', 'error'); return; }
            if (!tipoManual) { showToast('Debe seleccionar Tipo marca manual.', 'error'); return; }
            ...
            let obj = {
                ...
                fecha           : fecha,
                hora            : hora,
                evento          : tipo,                // Entrada/Salida
                tipoMarcaManual : tipoManual,
                comentario      : $('#newComentario').val() || '',
                nombreEmpleado  : $('#newNombreEmpleado').val() || '',
                hashcode        : $('#newHashcode').val() || ''
            };
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.exito) {
                        showToast(response.mensaje || 'Marca creada correctamente.', 'success');
                        cargarRegistrosAjax();
                        bootstrap.Modal.getInstance(document.getElementById('newModal')).hide();
                    } else {
                        showToast(response && response.mensaje ? response.mensaje : 'No se pudo crear la marca.', 'error');
                    }
                },
                error: function () {
        ...
        $('#marcasTable tbody').on('click', '.deleteRow', function(){
            ...
            $('#deleteId').val(data.correlativo);
            $('#deleteRutEmpleado').val(data.rutEmpleado);
            $('#deleteNombreEmpleado').val('pepe');
