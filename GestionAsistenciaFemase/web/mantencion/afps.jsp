...
<body>
    <div class="site-container">
        <div class="main-header" style="position: relative">
            <h1>Administraci&oacute;n de AFPs</h1>
...
<div id="AfpsTableContainer"></div>
<script type="text/javascript">

    $(document).ready(function () {

        $('#AfpsTableContainer').jtable({       
            title: 'AFPs',
            paging: true, //Enable paging
            pageSize: 10, //Set page size (default: 10)
            sorting: true, //Enable sorting
            defaultSorting: 'afp_name ASC', //Set default sorting
            actions: {
                listAction: '<%=request.getContextPath()%>/servlet/AfpsController?action=list',
                createAction:'<%=request.getContextPath()%>/servlet/AfpsController?action=create',
                updateAction: '<%=request.getContextPath()%>/servlet/AfpsController?action=update'
        ...
        $('#LoadRecordsButton').click(function (e) {
            e.preventDefault();
            $('#AfpsTableContainer').jtable('load', {
                nombre: $('#nombre').val(),
                estado: $('#estado').val()
            });
        });
