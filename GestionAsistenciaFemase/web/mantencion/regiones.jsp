...
<body>
    <div class="site-container">
        <div class="main-header" style="position: relative">
            <h1>Administraci&oacute;n de Regiones</h1>
...
<div id="RegionesTableContainer"></div>
<script type="text/javascript">

    $(document).ready(function () {

        $('#RegionesTableContainer').jtable({       
            title: 'Regiones',
            paging: true, //Enable paging
            pageSize: 10, //Set page size (default: 10)
            sorting: true, //Enable sorting
            defaultSorting: 'short_name ASC', //Set default sorting
            actions: {
                listAction: '<%=request.getContextPath()%>/RegionesController?action=list',
                createAction:'<%=request.getContextPath()%>/RegionesController?action=create',
                updateAction: '<%=request.getContextPath()%>/RegionesController?action=update'
        ...
        $('#LoadRecordsButton').click(function (e) {
            e.preventDefault();
            $('#RegionesTableContainer').jtable('load', {
                nombre: $('#nombre').val()
            });
        });
