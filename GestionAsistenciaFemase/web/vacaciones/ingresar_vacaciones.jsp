  ...
  <select class="form-control" id="idAusencia" name="idAusencia"   
    ...
    <%
        ...
        while(iteraAusencias.hasNext() ) {
            ...
            %>
                <option value="<%=ausencia.getId()%>" selected><%=label%></option>
            <%}
            }
        %>
  </select>
...
<div class="form-group permiteHora required" data-fid="permiteHora">
  ...
  <input type="hidden" name="permiteHora" id="permiteHora" value="N">
...
<div class="form-group fechaInicioAsStr required" data-fid="fechaInicioAsStr">
  ...
  data-datepicker-startView="1" />
    ...
    <span class="form-group permiteHora required">
    <input type="hidden" name="soloHoraInicio" id="soloHoraInicio">
    <input type="hidden" name="soloMinsInicio" id="soloMinsInicio">
    </span></div>
</div>
...
<div class="form-group fechaFinAsStr required" data-fid="fechaFinAsStr">
  ...
  data-datepicker-format="dd-mm-yyyy"
  data-datepicker-startView="1" />
    <span class="form-group soloHoraInicio required"><span id="errmsg2"></span>
    </span> 
    <input type="hidden" name="soloHoraFin" id="soloHoraFin">
    <input type="hidden" name="soloMinsFin" id="soloMinsFin">
     </div>
</div>
<div id="error" style="color:#F00"></div>
    <div class="form-group dias_solicitados" data-fid="dias_solicitados">
   	
        <input type="hidden" name="dias_solicitados" id="dias_solicitados">
    </div>
...
action="<%=request.getContextPath()%>/vacaciones/admin_vacaciones.jsp">  
        ...
        $(document).ready(function() {
			...
			//$("#rutEmpleado").empty();
                        ...
                        for (i=0; i < response.length; i++) {
                            var auxDE = "";
                            if (response[i].diasEspeciales === 'S') auxDE = '[Dias especiales] ';
                            var auxNombre = '['+response[i].rut+'] ' + response[i].nombres + 
                                ' ' + response[i].apePaterno 
                                + ' (' + response[i].nombreTurno + ').' + auxDE + 'Saldo dias vacaciones= '+response[i].saldoDiasVacaciones;    
