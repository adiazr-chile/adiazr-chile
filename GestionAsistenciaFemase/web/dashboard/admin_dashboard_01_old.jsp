...
<head>
  <meta charset="UTF-8">
  <title>Dashboard Alternativo Asistencia (con Chart.js)</title>
  ...
  <style>
    ...
    .card {
      ...
      border-radius: 8px;
      font-size: 1.17rem;
      box-shadow: 0 3px 6px rgba(32,63,144,0.10);
      position: relative;
      margin-bottom: 8px;
    }
    .card.atrasos { background:#f5892c; }
    .card.sinmarcar { background:#e53935; }
    .card.ausencias { background:#028a79; }
    .card .number {
      font-size: 2.3em;
      font-weight: bold;
      margin-bottom: 6px;
    ...
    table {
      ...
      border-radius: 8px;
      box-shadow: 0 1px 4px rgba(40,40,40,0.06);
      overflow: hidden;
      font-size: 1rem;
      margin-top: 8px;
    }
...
<body>
  <div class="dashboard">
    ...
    <div class="cards">
      ...
      <div class="card atrasos">
        <div>Empleados con atrasos</div>
        <div class="number">3</div>
      </div>
      <div class="card sinmarcar">
      ...
      <div class="tables-section">
        ...
        <div>
          ...
          <table>
            ...
            <tbody>
              <!-- Llena dinámicamente desde el backend -->
              <tr>
  ...
  <script>
    // Reemplaza estos datos por los que recibas de tu backend
    const dataPie = {
      labels: ['Vacaciones', 'Licencia médica', 'Permiso'],
      datasets: [{
        data: [10, 5, 10],
        backgroundColor: [
          '#00c776', // Vacaciones
          '#ffb200', // Licencia médica
          '#2176ff'  // Permiso
        ]
      }]
    };
    ...
    const configPie = {
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: false, // Ocultamos, usamos la nuestra personalizada
          }
        }
      }
    };
    ...
    window.addEventListener('DOMContentLoaded', () => {
      const pieChart = new Chart(document.getElementById('ausenciasPie'), configPie);
      ...
      function renderPieLegend() {
        ...
        dataPie.labels.forEach((label, i) => {
          legend.innerHTML += `<div>
            <span style="display:inline-block;width:15px;height:15px;margin-right:8px;background:${dataPie.datasets[0].backgroundColor[i]};border-radius:2px;"></span>
            ${label} (${dataPie.datasets[0].data[i]})
          </div>`;
        });
      }
      renderPieLegend();
